...
def tree_to_df(file_path_name, tree_name, n_jobs):
    """"
    Imports a root file into python and converts it to a data frame
            
    """
    executor = ThreadPoolExecutor(n_jobs)
    file = uproot.open(""+file_path_name+":"+tree_name+"",decompression_executor=executor,
                                  interpretation_executor=executor)
    df = file.arrays(library="pd",decompression_executor=executor,
                                  interpretation_executor=executor)
    return df

def selec_dis_from_mean_std(df, variable_in_df, num_of_sigma_left, num_of_sigma_right):
    """"
    Selects data from a dataframe in some standard deviations
    
    Args:
        df (Pandas.Dataframe)   : The input dataframe
        variable_in_df (string) : a column name in df
        num_of_sigma (int)      : the number of sigmas to select the data
        
    Returns
        selec_df (Pandas.Dataframe): A subset of df after the application of selection
            
    """
    mean = df[variable_in_df].mean()
    std  = df[variable_in_df].std()
    selec_df = df[(df[variable_in_df]> (mean-num_of_sigma_left*std) ) &
                  (df[variable_in_df]< (mean+num_of_sigma_right*std ) )]
    return selec_df
  

def fragmentation_selection(df, layer_info, frag_option):
    '''
    The function applies the MC selection on detector part to check whether a nuclei has frgmented or not
...
def labelled(df, label_frag, label_non_frag):
    '''
    The function applies the fragmentation_selection function and labels the data
    ...
    '''
    df_frag = fragmentation_selection(df, 13, 'fragmented')
    df_non_frag = fragmentation_selection(df, 13, 'non-fragmented')
    df_frag['label']= np.zeros(df_frag.shape[0])
    df_frag['label'] = df_frag['label'].replace(0,label_frag)
    df_non_frag['label']= np.zeros(df_non_frag.shape[0])
    df_non_frag['label'] = df_non_frag['label'].replace(0,label_non_frag)
...
def selection_bounds_based(df, variable_name, lower_bound, upper_bound):
    '''
    Returns data from dataframe by applying selection on a variable
    ...
    '''
    return df[(df[variable_name]>=lower_bound) & (df[variable_name]<upper_bound)]

def selelction_and_labelled(df, variable_name, lower_bound, upper_bound, label_frag, label_non_frag):
    '''
    This function combines selection_bounds_based with labelled
    Args:
        df (Pandas.Dataframe)   : The input dataframe
        variable_in_df (string) : a column name in df
        lower_bound (float)     : the lower bound on the variable
    ...
    '''
    df = selection_bounds_based(df, variable_name, lower_bound, upper_bound)
    df_frag, df_non_frag = labelled(df, label_frag, label_non_frag)
    return df_frag, df_non_frag

def tree_df_selec_lab(file_path_name, tree_name, n_jobs, variable_name, lower_bound, upper_bound, label_frag, label_non_frag):
    df = tree_to_df(file_path_name, tree_name, n_jobs)
    df_frag, df_non_frag = selelction_and_labelled(df, variable_name, lower_bound, upper_bound, label_frag, label_non_frag)
    del df
    return df_frag, df_non_frag
